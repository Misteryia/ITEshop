...
CATALOG_URL = "http://catalog:8003"
CART_URL = "http://cart:8004"
...
async def read_home(request: Request):
    try:
        ...
        email = None
    print("DEBUG: frontend check cookies: email - ", email)
...
async def login_action(request: Request, email: str = Form(...), password: str = Form(...), client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    print("DEBUG: frontend in post login")
    try:
        response = await client.post(
            f"{AUTH_URL}/login",
            json={"email": email, "password": password}
...
async def register_seller(request: Request, email: str = Form(...), password: str = Form(...), shop_name: str = Form(...), inn: str = Form(None), description: str = Form(None), client: httpx.AsyncClient = Depends(get_http_client)):
    """Отправить запрос на регистрацию продавца в auth."""
    try:
        response = await client.post(
            f"{AUTH_URL}/register/seller",
            json={
                "email": email,
                "password": password,
                "shop_name": shop_name,
...
async def seller_add_product_page(request: Request, client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    try:
        ...
        return RedirectResponse(url="/login", status_code=303)
    # Проверяем роль через auth
    role_resp = await client.get(f"{AUTH_URL}/role?email={email}")
...
async def seller_add_product(request: Request, client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    if "file" in form and form["file"]:
        ...
        response = await client.post(f"{CATALOG_URL}/api/products/bulk_upload", files=files)
        ...
        # Получаем seller_id через auth
        seller_resp = await client.get(f"{AUTH_URL}/get_user_id?email={email}")
        ...
        response = await client.post(f"{CATALOG_URL}/products", json=product_data)
...
async def seller_edit_product_page(request: Request, id: int, client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    product_resp = await client.get(f"{CATALOG_URL}/api/get_product?id={id}")
...
async def seller_update_product(request: Request, id: int = Form(...), name: str = Form(...), price: float = Form(...), description: str = Form(None), stock: int = Form(None), client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    response = await client.put(f"{CATALOG_URL}/edit_product/{id}", json=update_data, headers={"Auth": f"Bearer {jwt_token}"})
    result = response.json()
...
@app.get("/seller/metr", response_class=HTMLResponse)
@log_kafka
@api_metr()
@trace_function(name="seller_metr_page", include_request=True)
async def seller_metr_page(request: Request, client: httpx.AsyncClient = Depends(get_http_client)):
    jwt_token = request.cookies.get("access_token")
    if not jwt_token:
        return RedirectResponse(url="/login", status_code=303)
    try:
    ...
    return templates.TemplateResponse("seller_metr.html", {"request": request})
...
async def seller_delete_product(request: Request, id: int = Form(...), client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    response = await client.delete(f"{CATALOG_URL}/products/{id}", headers={"Auth": f"Bearer {jwt_token}"})
...
async def edit_profile_page(request: Request, client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    resp = await client.get(f"http://auth:8001/profile?email={email}")
...
async def edit_profile_action(request: Request, client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    if role == "seller":
        ...
        inn = form.get("inn")
        description = form.get("description")
        # Отправляем PATCH/POST в auth для обновления данных продавца
        resp = await client.post(
            f"http://auth:8001/profile/edit_seller",
            json={
                "email": email,
                "shop_name": shop_name,
                "inn": inn,
        ...
        resp = await client.post(
            f"http://auth:8001/profile/edit_user",
            json={
                "email": email,
...
async def admin_users_page(request: Request, search: str = '', role: str = '', client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    if role:
        params['role'] = role
    resp = await client.get(f"http://auth:8001/api/users", params=params)
...
async def admin_delete_product(request: Request, client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    jwt_token = request.cookies.get("access_token")
    headers = {"Auth": f"Bearer {jwt_token}"} if jwt_token else {}
    resp = await client.post(
        f"http://catalog:8003/admin_delete_product",
        json={"id": product_id},
        headers=headers
    )
    if resp.status_code != 200:
...
@app.get("/admin/metr", response_class=HTMLResponse)
@log_kafka
@api_metr()
@trace_function(name="admin_metr_page", include_request=True)
async def admin_metr_page(request: Request):
...
async def admin_delete_user(request: Request, client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    headers = {"Auth": f"Bearer {jwt_token}"} if jwt_token else {}
    resp = await client.post(
        f"http://auth:8001/admin_delete_user",
        json={"id": user_id},
        headers=headers
    )
    if resp.status_code != 200:
...
async def admin_orders_page(request: Request, search: str = '', status: str = '', client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    resp = await client.get("http://auth:8001/admin/orders", params=params, headers=headers)
...
async def admin_update_order_status(request: Request, client: httpx.AsyncClient = Depends(get_http_client)):
    ...
    headers = {"Auth": f"Bearer {jwt_token}"} if jwt_token else {}
    resp = await client.post(
        "http://auth:8001/admin/update_order_status",
